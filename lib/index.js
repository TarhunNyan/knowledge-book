...
function compileProject(pathFrom, pathTo) {
    ...
    let pathsFile = [];
    pathsFile = fp.dir__getFileList_Recursive(pathFrom);
    ...
    _.for__simple(isMakeUpdate, (i, el) => {
        ...
        const data = compileProject__convert(pathF, pathT, { pathStatic: pathS });
...
function compileProject__generatePathStatic(pathNew) {
    ...
    arr = ['./', ...arr, 'static', 'style.css'];
    return fp.path__join(arr);
}
...
function compileProject__convert(pathFrom, pathTo, options) {
    let data = fp.file__read(pathFrom);
    const extFrom = fp.path__getExt(pathFrom), extTo = fp.path__getExt(pathTo);

    if (extFrom === '.md' && extTo === '.html') { return `<link rel="stylesheet" href="${options.pathStatic}">` + compileProject__convert_mdToHTML(data); }
...
function compileProject__convert_mdToHTML(data) {
    ...
    return `<script>document.addEventListener('click', (event)=>{const anchor=event.target.closest('a');if (anchor != null && anchor.getAttribute('href')[0] === '#') {event.preventDefault();let newHref = location.href;const index = newHref.indexOf('#');if (index != -1) {newHref = newHref.slice(0, index);}newHref += event.target.closest('a').getAttribute('href');location.replace(newHref);}})</script>input type="checkbox" id="isClose" />input type="checkbox" id="isHome" />div class="left-menu">div class="control-panel">label for="isClose">div class="isClose icon"></div></label><!-- <label for="isHome">div class="isHome icon"></div></label> --></div><div class="left-side-menu">${menu}</div></div><div class="main">${data}</div>`
}
...
if (_.if__isStringContent(options['folder-from']) && _.if__isStringContent(options['folder-to'])) { console.log('Компиляция...'); compileProject('./md', './dist'); }
